	...
	/*
      ...
      public static function pageBg()
	  {
          if(self::$pagedata->{'custom_bg' . Lang::$lang}) {
			  return ' style="background-image: url(' . UPLOADURL . '/' . self::$pagedata->{'custom_bg' . Lang::$lang} . '); background-size: cover;"';
		  }
      } 
      ...
      public static function pageHeading()
	  {
		  if(File::is_File(UPLOADS . "/images/" . self::$pagedata->{'slug' . Lang::$lang} . '.jpg')) {
			  $bg = ' style="background-image: url(' . UPLOADURL . '/images/' . self::$pagedata->{'slug' . Lang::$lang} . '.jpg);no-repeat center center/cover;"';
		  } elseif(File::is_File(UPLOADS . '/images/default-heading.jpg')) {
			  $bg = ' style="background-image: url(' . UPLOADURL . '/images/default-heading.jpg);no-repeat center center/cover;"';
		  } else {
			  $bg = '';
		  }
		  ...
		  if ($matches[1]) {
			  ...
			  foreach ($matches[1] as $k => $row) {
				  ...
				  if($items[1] == "plugin") {
					  ...
					  if($is_builder) {
						   $html = '<div data-mode="readonly" data-plugin-id="' . $items[3] . '" data-plugin-plugin_id="' . $items[2] . '" data-plugin-alias="' . $items[0] . '">' . $contents . '</div>';
					  } else {
						  $html = $contents;
					  }
				  } else {
					  ...
					  if($is_builder) {
						   $html = '<div data-mode="readonly" data-module-group="' . $group[0] . '" data-module-module_id="' . $items[2] . '" data-module-id="' . $items[3] . '" data-module-alias="' . $items[0] . '">' . $contents . '</div>';
					  } else {
						  $html = $contents;
					  }
				  }
      ...
      public static function parseNewContentData($body, $is_builder = false)
      {
