  ...
  class Auth
  {
      ...
      public function userLogin()
      {
		  ...
		  if (!Logger::loginAgain($timeleft)) {
			  ...
			  $rules = array(
				  'email' => array('required|string', Lang::$word->LOGIN),
				  'password' => array('required|string', Lang::$word->M_PASSWORD),
				  );
			  ...
			  $status = $this->checkStatus($safe->email, $safe->password);
          ...
          if (empty(Message::$msgs) && $status == "y") {
			  ...
			  $json['message'] = Lang::$word->LOGIN_R7;
			  $json['redirect'] = Url::url('/' . App::Core()->system_slugs->account[0]->{'slug' . Lang::$lang});
