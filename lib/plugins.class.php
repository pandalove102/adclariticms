<?php
   ...
   * @copyright 2016
   * @version $Id: plugins.class, v1.00 2016-04-20 18:20:24 gewa Exp $
   */
  ...
  class Plugins
  {
      ...
      public static function loadPluginFile($items)
      {
		  ...
		  if(File::is_File(FPLUGPATH . $items[0] . "/themes/" . App::Core()->theme . "/index.tpl.php")) {
			  $contents = Utility::getSnippets(FPLUGPATH . $items[0] . "/themes/" . App::Core()->theme . "/index.tpl.php", $data = ["plugin_id" => $items[1], "id" => $items[2]]);
		  } else {
			  $contents = Utility::getSnippets(FPLUGPATH . $items[0] . "/index.tpl.php", $data = ["plugin_id" => $items[1], "id" => $items[2]]);
		  }
