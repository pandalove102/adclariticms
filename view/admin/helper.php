<?php
  ...
  if (isset($_GET['doAction'])):
      switch ($_GET['page']) :
				  ...
				  foreach ($data as $row):
					  if(Validator::get('is_builder')) :
						  $item = array(
							  ...
							  'href' => Url::url("/" . $row->{'slug' . Lang::$lang}), 
							  'id' => $row->id
						  );
					  else:
						  $item = array(
							  'title' => $row->{'title' . Lang::$lang}, 
							  'value' => Url::url("/" . $row->{'slug' . Lang::$lang}), 
							  'id' => $row->id
						  );
					  endif;
					 $list[] = $item;
          ...
          /* == Get Builder Plugin == */
          case "builderPlugin":
		      if($row = Db::run()->first(Plugins::mTable, array("id", "title" . Lang::$lang . " as title", "plugalias", "groups", "plugin_id", "show_title"), array("id" => Filter::$id))):
				  if(File::is_File(FPLUGPATH . $row->plugalias . "/themes/" . App::Core()->theme . "/index.tpl.php")) :
				     $content = Utility::getSnippets(FPLUGPATH . $row->plugalias . "/themes/" . App::Core()->theme . "/index.tpl.php", $data = (array)$row);
				  else:
				     $content = Utility::getSnippets(FPLUGPATH . $row->plugalias . "/index.tpl.php", $data = (array)$row);
				  endif;
				  
				  $assets = Plugins::parsePluginAssets('%%' . $row->plugalias . '|plugin|0|0"%%');
				  
                  $json['type'] = "success";
				  $json['html'] = $content;
				  $json['assets'] = $assets;
				  $json['assets_id'] = $row->groups;
			  else:
                  $json['type'] = "error";
				  $json['html'] = '';  
			  endif;
