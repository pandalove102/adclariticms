<?php
  ...
  switch ($action):
      ...
      case "saveSlideData":
          if (Filter::$id):
		      $data = array(
				 'image' => $_POST['image'] ? Validator::sanitize($_POST['image']) : 'NULL',
      ...
      case "getShape":
		  ...
		  $tpl->template = 'loadShapes.tpl.php'; 
      ...
      case "getIcons":
		  $tpl = App::View(APLUGPATH . 'slider/snippets/'); 
		  $tpl->data = File::findFiles(UPLOADS . '/svg',array('fileTypes'=>array('svg')));
		  $tpl->template = 'loadIcons.tpl.php'; 
          $json['html'] = $tpl->render();
          print json_encode($json);
          break;
      ...
      case "getButtons":
		  $tpl = App::View(APLUGPATH . 'slider/snippets/'); 
		  $tpl->template = 'loadButtons.tpl.php'; 
          $json['html'] = $tpl->render();
          print json_encode($json);
          break;
      ...
      case "duplicateSlide":
          if($row = Db::run()->first(Slider::dTable, null, array('id' => Filter::$id))):
		      $data = array(
				 ...
				 'mode' => $row->mode,
				 'html' => str_replace("item_" . Filter::$id, "item_" . intval(Filter::$id+1), $row->html),
			  );
