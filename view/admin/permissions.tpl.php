<?php
   ...
   * @copyright 2016
   * @version $Id: permissions.tpl.php, v1.00 2016-05-05 10:12:05 gewa Exp $
   */
  if (!defined("_WOJO"))
    ...
    foreach ($this->result as $type => $rows):
        ...
        foreach ($rows as $i => $row):
            if (isset($row->mode) and $row->mode == "add") {
                ...
                echo '<div class="wojo small slider checkbox" data-id="' . $row->id . '"><input type="checkbox" data-val="' . $row->active . '" value="' . $row->id . '" name="view-' . $row->id . '" ' . $is_owner . $checked . '><label><span data-tooltip="' . $row->description . '"><i class="icon question sign"></i></span></label></div> ';
            }
        endforeach;
        echo '</td>';
        ...
        foreach ($rows as $row):
            if (isset($row->mode) and $row->mode == "approve") {
                $checked = ($row->active == 1) ? ' checked="checked"' : null;
                $is_owner = ($this->role->code == "owner") ? ' disabled="disabled"' : null;
                echo '<div class="wojo slider slider checkbox" data-id="' . $row->id . '"><input type="checkbox" data-val="' . $row->active . '" value="' . $row->id . '" name="view-' . $row->id . '" ' . $is_owner . $checked . '><label><span data-tooltip="' . $row->description . '"><i class="icon question sign"></i></span></label></div>';
            }
        endforeach;
        echo '</td>';
        ...
        foreach ($rows as $row):
            if (isset($row->mode) and $row->mode == "delete") {
                $checked = ($row->active == 1) ? ' checked="checked"' : null;
                $is_owner = ($this->role->code == "owner") ? ' disabled="disabled"' : null;
                echo '<div class="wojo small slider slider checkbox" data-id="' . $row->id . '"><input type="checkbox" data-val="' . $row->active . '" value="' . $row->id . '" name="view-' . $row->id . '" ' . $is_owner . $checked . '><label><span data-tooltip="' . $row->description . '"><i class="icon question sign"></i></span></label></div>';
            }
        endforeach;
        echo '</td>';
...
<div class="content-center"> <a href="<?php echo Url::url("/admin/permissions");?>" class="wojo button"><?php echo Lang::$word->CANCEL;?></a> </div>
<script type="text/javascript"> 
// <![CDATA[  
$(document).ready(function () {
    $("#pTable").on('click', '.wojo.slider.checkbox', function() {
        status = $(this).checkbox('is checked') ? 1 : 0;
        id = $(this).data('id');
...
<div class="row screen-block-3 tablet-block-2 mobile-block-1 phone-block-1 gutters align-center">
  ...
  <div class="column">
    <div class="wojo segment content-center"> <img src="<?php echo ADMINVIEW;?>/images/<?php echo $row->code;?>.svg" alt="">
      ...
      <div class="content-center"> <a href="<?php echo Url::url(Router::$path, "privileges/" . $row->id);?>" class="wojo icon circular basic negative small button"><i class="icon lock"></i></a> <a data-set='{"option":[{"doAction": 1, "processItem": 1,"page":"editRole", "editRole": 1,"id": <?php echo $row->id;?>}], "label":"<?php echo Lang::$word->M_INF01;?>", "url":"helper.php", "parent":"#item_<?php echo $row->id;?>", "action":"replace", "modalclass":"small"}' class="wojo icon circular basic small primary button addAction"> <i class="icon pencil"></i></a> </div>
    </div>
  </div>
  <?php endforeach;?>
</div>
