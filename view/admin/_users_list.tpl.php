...
<div class="row half-gutters align-middle">
  ...
  <div class="columns mobile-100 mobile-order-4">
    <div class="wojo small fluid inverted icon input">
      <input id="masterSearch" placeholder="<?php echo Lang::$word->SEARCH;?>" data-page="users" data-url="helper" type="text">
      <i class="icon find"></i> </div>
  </div>
  <div class="columns shrink mobile-50 mobile-order-3 mobile-content-right">
    <div class="wojo small icon buttons"> <a class="wojo icon active button"><i class="icon reorder"></i></a> <a href="<?php echo Url::url(Router::$path, "grid/");?>" class="wojo icon button"><i class="icon grid list"></i></a> <a href="<?php echo ADMINVIEW . '/helper.php?exportUsers';?>" class="wojo button"><?php echo Lang::$word->EXPORT;?></a> </div>
  </div>
</div>
...
<div class="wojo acard" id="item_<?php echo $row->id;?>">
  <div class="header">
    ...
    <div class="row align-middle">
      <div class="column">
        <div class="wojo horizontal inverted small list">
          ...
          <div class="item">
            <div class="header"> <span class="wojo caps light text"><?php echo Lang::$word->MEMBERSHIP;?></span> <span class="wojo caps text"><?php echo ($row->membership_id) ? '<a href="' . Url::url("/admin/memberships/edit/" . $row->membership_id) . '" class="white">' . $row->mtitle . '</a>' : '-/-';?></span> </div>
          </div>
        </div>
      </div>
      <div class="column shrink">
        <div class="wojo pointing top right dropdown"><i class="icon white horizontal ellipsis link"></i>
          <div class="menu">
            ...
            <a data-set='{"option":[{"trash": "trashUser","title": "<?php echo Validator::sanitize($row->fullname, "chars");?>","id":<?php echo $row->id;?>}],"action":"trash","parent":"#item_<?php echo $row->id;?>"}' class="item action"> <i class="icon trash"></i> <?php echo Lang::$word->TRASH;?></a>
            <?php endif;?>
          </div>
        </div>
      </div>
