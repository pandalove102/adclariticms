...
<div class="row half-gutters align-middle">
  ...
  <div class="columns content-right mobile-50 mobile-content-left mobile-order-2"> <a href="<?php echo Url::url(Router::$path, "new/");?>" class="wojo small secondary button"><i class="icon plus alt"></i><?php echo Lang::$word->PAG_SUB4;?></a> </div>
  <div class="columns mobile-100 mobile-order-4">
    <div class="wojo small fluid inverted icon input">
      <input id="masterSearch" placeholder="<?php echo Lang::$word->SEARCH;?>" data-page="pages" data-url="helper" type="text">
...
<div class="wojo segment">
  <table class="wojo sorting basic table">
    ...
    <tr id="item_<?php echo $row->id;?>">
      ...
      <td><a href="<?php echo Url::url(Router::$path, "edit/" . $row->id);?>" class="inverted"> <?php echo $row->{'title' . Lang::$lang};?></a></td>
      <td class="collapsing center aligned"><?php if($row->page_type == "contact"):?>
        ...
        <?php endif;?></td>
      <td class="collapsing"><a href="<?php echo Url::url(Router::$path, "edit/" . $row->id);?>" class="wojo icon circular basic positive button"><i class="icon positive note"></i></a> 
      <?php if($row->page_type == "normal"):?>
      <a data-set='{"option":[{"trash": "trashPage","title": "<?php echo Validator::sanitize($row->{'title' . Lang::$lang}, "chars");?>","id":<?php echo $row->id;?>}],"action":"trash","parent":"#item_<?php echo $row->id;?>"}' class="wojo icon circular basic negative button action"> <i class="icon negative trash"></i> </a>
      <?php else:?>
      <a class="wojo icon circular basic disabled button"> <i class="icon close"></i> </a>
      <?php endif;?>
