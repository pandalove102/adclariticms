      ...
      <div class="row">
        ...
        <div class="columns screen-<?php echo $row->space;?>0 tablet-<?php echo $row->space;?>0 mobile-100 phone-100">
          <div class="bottomwidget-wrap <?php echo ($row->alt_class) ? $row->alt_class : '';?>">
            ...
            <h4 class="wojo header"><?php echo $row->{'title' . Lang::$lang};?></h4>
            <?php endif;?>
            <?php if ($row->{'body' . Lang::$lang}):?>
            <div class="bottomwidget-body"><?php echo Url::out_url($row->{'body' . Lang::$lang});?></div>
            <?php endif;?>
            <?php if ($row->jscode):?>
            <script>
            ...
            <?php endif;?>
            <?php if ($row->system):?>
            <?php echo Plugins::loadPluginFile(array($row->plugalias, $row->plugin_id, $row->plug_id));?>
            <?php endif;?>
          </div>
        </div>
        <?php if($this->layout->bottomCount > 1 and $this->layout->bcounter):?>
